...
- Valid API key and security key (defaults to ``IMEDNET_*`` env vars).
Usage:
1. Replace any remaining ``XXXXXXXXXX`` placeholders for `api_key` and
   `security_key` if environment variables are not set.
