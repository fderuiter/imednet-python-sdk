...
@patch("imednet.endpoints.intervals.build_paginator", wraps=build_paginator)
def test_list_with_study_key_and_filters(
    mock_builder, mock_interval, mock_paginator, mock_endpoint
):
...
@patch("imednet.endpoints.intervals.Paginator")
@patch("imednet.endpoints.intervals.Interval")
def test_list_with_only_study_key(mock_interval, mock_paginator, mock_endpoint):
...
@patch("imednet.endpoints.intervals.Paginator")
@patch("imednet.endpoints.intervals.Interval")
def test_list_uses_default_study_key(mock_interval, mock_paginator, mock_endpoint):
    mock_paginator.return_value = [{"id": 1}]
    mock_interval.from_json.side_effect = lambda x: x
...
def test_list_raises_value_error_if_no_study_key():
    ...
    endpoint = IntervalsEndpoint(client, ctx)
    with pytest.raises(ValueError):
        endpoint.list()


@patch("imednet.endpoints.intervals.Interval")
def test_get_returns_interval(mock_interval, mock_endpoint):
    mock_interval.from_json.return_value = {"id": 123}
    mock_endpoint._client.get.return_value.json.return_value = {"data": [{"id": 123}]}
    result = mock_endpoint.get("STUDY3", 123)
    assert result == {"id": 123}
    mock_interval.from_json.assert_called_once_with({"id": 123})


def test_get_raises_value_error_if_not_found(mock_endpoint):
    mock_endpoint._client.get.return_value.json.return_value = {"data": []}
    with pytest.raises(ValueError, match="Interval 456 not found in study STUDY3"):
