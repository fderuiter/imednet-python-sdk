...
from imednet_workflows.uat.engine import UATExecutionEngine, EditCheckResultStatus
from imednet.errors import ValidationError

def test_uat_engine_parses_rules():
    dd = DataDictionary(
        business_logic=[
            {"Name": "Rule1", "Status": "Active", "Form": "F1", "Variable": "V1", "Operator": ">=", "Value": "18"},
            {"Name": "Rule2", "Status": "Inactive", "Form": "F2"}
        ],
        choices=[], forms=[], questions=[]
    )
...
def test_uat_engine_subject_limit():
    ...
    rules = []
    for i in range(150):
        rules.append({"Name": f"Rule{i}", "Status": "Active", "Form": "F1", "Variable": f"V{i}", "Operator": ">=", "Value": "18"})
