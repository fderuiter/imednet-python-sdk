...
def mock_endpoint():
    ...
    return RecordRevisionsEndpoint(client, ctx)
...
@patch("imednet.endpoints.record_revisions.Paginator")
@patch("imednet.endpoints.record_revisions.RecordRevision")
def test_list_with_only_study_key(mock_record_revision, mock_paginator, mock_endpoint):
    mock_paginator.return_value = [{"id": 1}]
    mock_record_revision.from_json.side_effect = lambda x: x

    result = mock_endpoint.list(study_key="STUDY2")
    assert result == [{"id": 1}]
    assert mock_record_revision.from_json.call_count == 1


@patch("imednet.endpoints.record_revisions.Paginator")
@patch("imednet.endpoints.record_revisions.RecordRevision")
def test_list_uses_default_study_key(mock_record_revision, mock_paginator, mock_endpoint):
    mock_paginator.return_value = [{"id": 1}]
    mock_record_revision.from_json.side_effect = lambda x: x
...
@patch("imednet.endpoints.record_revisions.RecordRevision")
def test_get_returns_record_revision(mock_record_revision, mock_endpoint):
    mock_record_revision.from_json.return_value = {"id": 123}
    mock_endpoint._client.get.return_value.json.return_value = {"data": [{"id": 123}]}
    result = mock_endpoint.get("STUDY3", 123)
    assert result == {"id": 123}
    mock_record_revision.from_json.assert_called_once_with({"id": 123})


def test_get_raises_value_error_if_not_found(mock_endpoint):
    mock_endpoint._client.get.return_value.json.return_value = {"data": []}
    with pytest.raises(ValueError, match="Record revision 456 not found in study STUDY3"):
