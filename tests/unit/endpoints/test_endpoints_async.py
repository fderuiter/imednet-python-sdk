...
@pytest.mark.asyncio
async def test_async_get_record(dummy_client, context, async_paginator_factory):
    ep = records.RecordsEndpoint(dummy_client, context, async_client=dummy_client)
    captured = async_paginator_factory(records, [{"recordId": 1}])
...
@pytest.mark.asyncio
async def test_async_get_record_not_found(dummy_client, context, async_paginator_factory):
    ep = records.RecordsEndpoint(dummy_client, context, async_client=dummy_client)
