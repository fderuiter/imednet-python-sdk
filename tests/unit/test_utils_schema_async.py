"""Unit tests for utils schema async."""
...
async def test_async_schema_cache_refresh() -> None:
    """Test that async schema cache refresh asynchronously."""
    forms = MagicMock()
...
async def test_validate_record_and_batch_async() -> None:
    """Test that validate record and batch async asynchronously."""
    var = _make_var("age")
    sdk = MagicMock()
...
async def test_unknown_form_refreshes_and_raises() -> None:
    """Test that unknown form refreshes and raises asynchronously."""
    var = _make_var("age")
    sdk = MagicMock()
    sdk.async_get_variables = AsyncMock(return_value=[var])
    validator = AsyncSchemaValidator(sdk)
