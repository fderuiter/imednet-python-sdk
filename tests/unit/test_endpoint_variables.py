from unittest.mock import Mock, patch

import pytest
from imednet.endpoints.helpers import build_paginator
from imednet.endpoints.variables import VariablesEndpoint


@pytest.fixture
def mock_endpoint():
    client = Mock()
    ctx = Mock()
    ctx.default_study_key = "DEF123"
    return VariablesEndpoint(client, ctx, 200)


@patch("imednet.endpoints.variables.Paginator")
@patch("imednet.endpoints.variables.Variable")
@patch("imednet.endpoints.variables.build_paginator", wraps=build_paginator)
def test_list_with_study_key_and_filters(
    mock_builder, mock_variable, mock_paginator, mock_endpoint
):
    mock_paginator.return_value = [{"id": 1}, {"id": 2}]
    mock_variable.from_json.side_effect = lambda x: x

    result = mock_endpoint.list(study_key="STUDY1", foo="bar")
    mock_builder.assert_called_once()
    assert mock_paginator.called
    assert result == [{"id": 1}, {"id": 2}]
    args, kwargs = mock_paginator.call_args
    assert kwargs["page_size"] == 200
    assert mock_variable.from_json.call_count == 2


@patch("imednet.endpoints.variables.Paginator")
@patch("imednet.endpoints.variables.Variable")
def test_list_with_only_study_key(mock_variable, mock_paginator, mock_endpoint):
    mock_paginator.return_value = [{"id": 1}]
    mock_variable.from_json.side_effect = lambda x: x

    result = mock_endpoint.list(study_key="STUDY2")
    assert result == [{"id": 1}]
    assert mock_variable.from_json.call_count == 1
    args, kwargs = mock_paginator.call_args
    assert kwargs["page_size"] == 200


@patch("imednet.endpoints.variables.Paginator")
@patch("imednet.endpoints.variables.Variable")
def test_list_uses_default_study_key(mock_variable, mock_paginator, mock_endpoint):
    mock_paginator.return_value = [{"id": 1}]
    mock_variable.from_json.side_effect = lambda x: x

    result = mock_endpoint.list()
    assert result == [{"id": 1}]
    assert mock_variable.from_json.call_count == 1
    args, kwargs = mock_paginator.call_args
    assert kwargs["page_size"] == 200


@patch("imednet.endpoints.variables.Paginator")
@patch("imednet.endpoints.variables.Variable")
def test_custom_page_size(mock_variable, mock_paginator, mock_endpoint):
    mock_paginator.return_value = []
    mock_variable.from_json.side_effect = lambda x: x

    mock_endpoint.list(page_size=75)
    args, kwargs = mock_paginator.call_args
    assert kwargs["page_size"] == 75


def test_list_raises_value_error_if_no_study_key():
    client = Mock()
    ctx = Mock()
    ctx.default_study_key = None
    endpoint = VariablesEndpoint(client, ctx)
    with pytest.raises(ValueError):
        endpoint.list()


@patch("imednet.endpoints.variables.Variable")
def test_get_returns_variable(mock_variable, mock_endpoint):
    mock_variable.from_json.return_value = {"id": 123}
    mock_endpoint._client.get.return_value.json.return_value = {"data": [{"id": 123}]}
    result = mock_endpoint.get("STUDY3", 123)
    assert result == {"id": 123}
    mock_variable.from_json.assert_called_once_with({"id": 123})


def test_get_raises_value_error_if_not_found(mock_endpoint):
    mock_endpoint._client.get.return_value.json.return_value = {"data": []}
    with pytest.raises(ValueError, match="Variable 456 not found in study STUDY3"):
        mock_endpoint.get("STUDY3", 456)
