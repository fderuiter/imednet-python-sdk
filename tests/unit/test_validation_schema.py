...
def test_schema_module_exports() -> None:
    # First ensure we import it, and catch the warning so it doesn't fail strict CI
    with pytest.warns(DeprecationWarning, match="imednet.validation.schema is deprecated; use imednet.validation.cache"):
...
def test_schema_module_deprecation_warning() -> None:
    import imednet.validation.schema as schema
