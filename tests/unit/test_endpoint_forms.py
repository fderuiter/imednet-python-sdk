...
@patch("imednet.endpoints.forms.Paginator")
@patch("imednet.endpoints.forms.Form")
def test_list_with_only_study_key(mock_form, mock_paginator, mock_endpoint):
...
@patch("imednet.endpoints.forms.Paginator")
@patch("imednet.endpoints.forms.Form")
def test_list_uses_default_study_key(mock_form, mock_paginator, mock_endpoint):
    mock_paginator.return_value = [{"id": 1}]
    mock_form.from_json.side_effect = lambda x: x
...
@patch("imednet.endpoints.forms.Form")
def test_get_returns_form(mock_form, mock_endpoint):
    ...
    mock_form.from_json.assert_called_once_with({"id": 123})


def test_get_raises_value_error_if_not_found(mock_endpoint):
    mock_endpoint._client.get.return_value.json.return_value = {"data": []}
    with pytest.raises(ValueError, match="Form 456 not found in study STUDY3"):
