...
def dashboard_server():
    ...
    env = os.environ.copy()
    env["PYTHONPATH"] = os.path.abspath("packages/core/src") + os.pathsep + \
                       os.path.abspath("packages/plugins-workflows/src") + os.pathsep + \
    ...
    if not wait_for_port(port):
        ...
        proc.kill()
        raise RuntimeError(f"Streamlit failed to start on port {port}. stdout: {stdout.decode()}, stderr: {stderr.decode()}")
