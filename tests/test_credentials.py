...
from imednet.credentials import (
    list_credentials,
    resolve_credentials,
    save_credentials,
)
...
def test_save_and_load(tmp_path):
    ...
    save_credentials("a", "b", "c", "Study", "pwd", path)
    creds = list_credentials("pwd", path)
    assert creds == [
        {
            "api_key": "a",
            "security_key": "b",
...
def test_resolve_credentials_from_env(monkeypatch):
    ...
    assert resolve_credentials() == ("env_api", "env_sec", "ENVSTUDY")


def test_resolve_credentials_from_file(tmp_path, monkeypatch):
    path = tmp_path / "cred.enc"
    save_credentials("file_api", "file_sec", "FILESTUDY", "Study", "pwd", path)
    ...
    monkeypatch.setenv("IMEDNET_CRED_PASSWORD", "pwd")
    assert resolve_credentials("FILESTUDY") == (
        "file_api",
        "file_sec",
        "FILESTUDY",
    )
