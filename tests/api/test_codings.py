...
def test_list_codings_with_params(codings_client, client):
    ...
    expected_params = {
        ...
        "size": "20",
        "sort": "recordId,desc",
        "filter": "dictionaryName=MedDRA",
    }
    route = respx.get(mock_url, params=expected_params).mock(
        return_value=Response(200, json=mock_response)
    )

    response = codings_client.list_codings(
        study_key=study_key, page=0, size=20, sort="recordId,desc", filter="dictionaryName=MedDRA"
    )
