...
*(Note: If you are running the project from source or a local clone, make sure to first install dependencies with `uv sync`. Then, prefix commands with `uv run`, e.g., `uv run imednet --help`)*
...
If building documentation with `make docs` fails with `Command not found: sphinx-apidoc`, run `uv sync --all-extras` first to install all necessary documentation plugins and dependencies.

**ModuleNotFoundError when running the CLI locally**
If you are running the `imednet` CLI from source (e.g., `uv run imednet`) and see a `ModuleNotFoundError` (such as `No module named 'imednet'`), ensure you have installed the project dependencies by running `uv sync` in the project root.
