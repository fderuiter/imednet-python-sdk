...
class CentralizedMapper:
    ...
    def map_record(
        ...
        sk = mapped.get('study_key') or "UNKNOWN"
        if sk not in self._pipeline_cache:
            self._pipeline_cache[sk] = EnrichmentPipeline(
                StudyConfiguration(
                    studyKey=sk,
                    version="1.0",
        ...
        mapped = self._pipeline_cache[sk].process(mapped)
