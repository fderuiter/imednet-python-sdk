...
from ..integrations.export import export_to_csv, export_to_duckdb, export_to_excel, export_to_json, export_to_long_sql, export_to_parquet, export_to_sql, export_to_sql_by_form
...
def app(args=None):
    ...
    parser.add_argument("--high-contrast", action="store_true", help="Enable high-contrast mode for accessibility.")
    ...
    # dashboard
    dash_parser = subparsers.add_parser("dashboard", help="Launch the iMednet Streamlit reporting dashboard.")
    dash_parser.add_argument("-p", "--port", type=int, default=8501, help="Port to run the dashboard on.")
    dash_parser.add_argument("--no-browser", action="store_true", help="Suppress automatic browser launch.")
    dash_parser.set_defaults(func=lambda a: run_dashboard(port=a.port, no_browser=a.no_browser))

    
    # workflows
    try:
        from imednet_workflows.cli import setup_parser as wf_setup, setup_subject_parser as subj_setup
        wf_setup(subparsers)
        subj_setup(subparsers)
    except ImportError:
        # Fallbacks when plugin not installed
        wf_parser = subparsers.add_parser("workflows", help="Execute opinionated business logic (requires imednet-workflows).")
        ...
        sync_parser = wf_sub.add_parser("sync-worker", help="Run a background cache synchronization worker.")
...
app.add_typer = _dummy_add_typer
app.command = _dummy_command
app.callback = _dummy_callback
