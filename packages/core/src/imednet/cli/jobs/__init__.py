...
def setup_parser(subparsers):
    ...
    status_parser.set_defaults(func=lambda args: job_status(study_key=args.study_key, batch_id=args.batch_id))
    ...
    @with_sdk
    def job_wait(sdk: ImednetSDK, study_key: str, batch_id: str, interval: int, timeout: int) -> None:
        with fetching_status("job result", study_key):
            job = sdk.poll_job(study_key, batch_id, interval=interval, timeout=timeout)
        print(job.model_dump())
    wait_parser.set_defaults(func=lambda args: job_wait(study_key=args.study_key, batch_id=args.batch_id, interval=args.interval, timeout=args.timeout))
