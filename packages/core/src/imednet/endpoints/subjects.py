...
class SubjectsEndpoint(SubjectsOperationDef, EdcListGetEndpoint[Subject, ClientT]):  # type: ignore[misc]
    ...
    @overload
    def list_by_site(
        self: "SubjectsEndpoint[RequestorProtocol]", study_key: str, site_id: str | int
    ) -> List[Subject]: ...

    @overload
    def list_by_site(
        self: "SubjectsEndpoint[AsyncRequestorProtocol]", study_key: str, site_id: str | int
    ) -> Awaitable[List[Subject]]: ...

    def list_by_site(
        self, study_key: str, site_id: str | int
    ) -> Union[List[Subject], Awaitable[List[Subject]]]:
        """TODO: Add docstring."""
    ...
    def async_list_by_site(self, study_key: str, site_id: str | int) -> Awaitable[List[Subject]]:
        ...
        warnings.warn(
            "async_list_by_site is deprecated, use list_by_site()", DeprecationWarning, stacklevel=2
        )
