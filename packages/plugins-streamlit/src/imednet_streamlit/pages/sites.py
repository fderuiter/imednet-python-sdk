    ...
    if df_q_with_site.empty:
        ...
        site_queries = (
            ...
            .agg(
                ...
                avg_days_open=(
                    "date_created",
                    lambda x: (timestamp_now - pd.to_datetime(x, utc=True)).dt.days.mean(),  # type: ignore[attr-defined, unused-ignore]
                ),
            )
            .reset_index()
        )
...
styled = page_df.style.map(_highlight_high_rate, subset=["query_rate"])  # type: ignore[arg-type, unused-ignore]
