    ...
    if query_rows:
        ...
        site_queries = (
            ...
            .agg(
                ...
                avg_days_open=(
                    "date_created",
                    lambda values: (now_utc - pd.to_datetime(values, utc=True)).dt.days.mean(),  # type: ignore[attr-defined, unused-ignore]
                ),
            )
            .reset_index()
        )
...
def _render_site_performance_tab(df_site_metrics: pd.DataFrame) -> None:
    ...
    st.dataframe(
        display.style.map(_highlight_high_rate, subset=["query_rate"]),  # type: ignore[arg-type, unused-ignore]
        use_container_width=True,
    )
