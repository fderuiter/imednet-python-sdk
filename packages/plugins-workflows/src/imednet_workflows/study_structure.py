...
def _build_study_structure(
    ...
    for interval in intervals:
        ...
        interval_forms = getattr(interval, "forms", [])
        for form_summary in interval_forms:
            if getattr(form_summary, "form_id", None) is not None:
                full_form = forms_by_id.get(form_summary.form_id)
                if full_form:
                    form_vars = variables_by_form_id.get(full_form.form_id, [])  # type: ignore[arg-type]
