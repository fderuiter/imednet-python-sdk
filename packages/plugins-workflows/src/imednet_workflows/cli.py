...
def setup_parser(subparsers):
    ...
    extract_parser = sub.add_parser("extract-records", help="Extract records based on criteria spanning subjects, visits, and records.")
    extract_parser.add_argument("study_key", help=STUDY_KEY_ARG)
    extract_parser.add_argument("--record-filter", action="append", help="Record filter criteria (e.g., 'form_key=DEMOG'). Repeat for multiple filters.")
    extract_parser.add_argument("--subject-filter", action="append", help="Subject filter criteria (e.g., 'subject_status=Screened'). Repeat for multiple filters.")
    extract_parser.add_argument("--visit-filter", action="append", help="Visit filter criteria (e.g., 'visit_key=SCREENING'). Repeat for multiple filters.")
    
    @with_sdk
    def extract_records(sdk: ImednetFacade, study_key: str, record_filter: list[str] | None = None, subject_filter: list[str] | None = None, visit_filter: list[str] | None = None) -> None:
    ...
    extract_parser.set_defaults(func=lambda args: extract_records(study_key=args.study_key, record_filter=args.record_filter, subject_filter=args.subject_filter, visit_filter=args.visit_filter))

    sync_parser = sub.add_parser("sync-worker", help="Run an idempotent background cache refresh worker.")
    sync_parser.add_argument("study_key", help=STUDY_KEY_ARG)
    sync_parser.add_argument("--interval", type=int, default=900, help="Polling interval in seconds.")
    sync_parser.add_argument("--once", action="store_true", help="Run a single sync cycle and exit.")
    
    @with_sdk
    def sync_worker(sdk: ImednetFacade, study_key: str, interval: int = 900, once: bool = False) -> None:
    ...
    sync_parser.set_defaults(func=lambda args: sync_worker(study_key=args.study_key, interval=args.interval, once=args.once))
    ...
    show_parser = state_sub.add_parser("show", help="Show the current high-water mark records and stream metadata.")
    show_parser.add_argument("-l", "--ledger-path", default="/var/lib/imednet/pipeline_ledger.json", help="Path to the pipeline ledger JSON file.")
    ...
    def show_state(ledger_path: str, study_key: str | None = None) -> None:
        ...
        print("Study Key | Stream Name | Last Timestamp (UTC) | Records Processed | Status | Error Message")
        for s_key, study_state in state.studies.items():
            ...
            for stream_name, stream_state in study_state.streams.items():
                has_data = True
                print(f"{s_key} | {stream_name} | {stream_state.last_timestamp.isoformat()} | {stream_state.records_processed} | {stream_state.last_run_status} | {stream_state.error_message or ''}")

        if not has_data:
            print("No ledger entries found matching filters.")
            
    show_parser.set_defaults(func=lambda args: show_state(ledger_path=args.ledger_path, study_key=args.study_key))
    
    set_parser = state_sub.add_parser("set", help="Manually set a high-water mark timestamp for a study and stream.")
    ...
    set_parser.add_argument("-t", "--timestamp", required=True, help="The ISO-8601 timestamp (UTC).")
    set_parser.add_argument("-r", "--records-processed", type=int, default=0, help="Number of records processed.")
    set_parser.add_argument("-l", "--ledger-path", default="/var/lib/imednet/pipeline_ledger.json", help="Path to the pipeline ledger JSON file.")
    
    def set_state(study_key: str, stream: str, timestamp: str, records_processed: int, ledger_path: str) -> None:
        try:
        ...
        try:
            ...
            print(f"Successfully set high-water mark for '{study_key}' -> '{stream}' to {dt.isoformat()}")
        except Exception as err:
            print(f"Failed to write ledger: {err}")
            sys.exit(1)
            
    set_parser.set_defaults(func=lambda args: set_state(study_key=args.study_key, stream=args.stream, timestamp=args.timestamp, records_processed=args.records_processed, ledger_path=args.ledger_path))

    reset_parser = state_sub.add_parser("reset", help="Reset (clear) high-water mark state for a study/stream.")
    reset_parser.add_argument("-s", "--study-key", required=True, help="The study key.")
    reset_parser.add_argument("-m", "--stream", help="The stream name. If omitted, all streams for the study will be reset.")
    reset_parser.add_argument("-l", "--ledger-path", default="/var/lib/imednet/pipeline_ledger.json", help="Path to the pipeline ledger JSON file.")
    ...
    reset_parser.set_defaults(func=lambda args: reset_state(study_key=args.study_key, stream=args.stream, ledger_path=args.ledger_path))

def setup_subject_parser(subparsers):
    subj_parser = subparsers.add_parser("subject-data", help="Retrieve all data for a single subject.")
    ...
    subj_parser.set_defaults(func=lambda args: subject_data(study_key=args.study_key, subject_key=args.subject_key))
...
def state_app(args=None):
    ...
    reset_parser.add_argument("-l", "--ledger-path", default="/var/lib/imednet/pipeline_ledger.json")
