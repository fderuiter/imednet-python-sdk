...
def test_mongodb_wrapper():
    ...
    mock_sdk = MagicMock()
    with patch("imednet.integrations.sink_base.apply_enrichment_pipeline", lambda s, r: iter(r)), \
...
def test_snowflake_wrapper():
    ...
    mock_sdk = MagicMock()
    mock_config = SnowflakeSinkConfig(account="a", user="u", password="p", database="d", schema="s", warehouse="w")
    with patch("imednet.integrations.sink_base.apply_enrichment_pipeline", lambda s, r: iter(r)), \
         patch("imednet.integrations.sink_base.apply_quality_gate", lambda s1, s2, r, c: r), \
...
def test_snowflake_tmp_dir():
    ...
    with patch("imednet_sinks.warehouse._require_optional_dep") as mock_dep:
        ...
        mock_config = SnowflakeSinkConfig(account="a", user="u", password="p", database="d", schema="s", warehouse="w", stage="stg", table="tbl")
